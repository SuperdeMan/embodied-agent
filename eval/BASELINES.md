...
| 2026-08-08 | (感知 v1 合入) | tabletop_pick_place@v1 | 30/30 (100%) | PASS | **感知驱动闭环**（D015）：agent 只见 ColorBlob+深度反投影的感知物体（多相机后备+可见度门+运动学附着信念，估计误差 ~2mm），裁判仍读真值。脚本技能 + 感知输入与真值基线打平。运行：`uv run --group sim embodied sim --task eval/tasks/tabletop_pick_place_v1.yaml --perception color` |
